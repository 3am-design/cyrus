...
function setupCustomCursor() {
  ...
  const readPixelVar = (name, fallback) => {
  ...
  const getCursorSize = () => (
    state.mode === 'discover'
      ? readPixelVar('--cursor-discover-size', 78)
      : readPixelVar('--cursor-size', 11)
  );

  const getCursorTarget = (event) => {
    const discoverTarget = event.target.closest?.('.timeline-event-btn, .work-panel-link');
    if (discoverTarget) {
      return {
        mode: 'discover',
        x: event.clientX,
    ...
    const interactiveTarget = event.target.closest?.(
      'button:not(:disabled), a[href], [role="button"], .nav-link, .rail-toggle'
    );
  ...
  const paintCursor = () => {
    state.renderX += (state.targetX - state.renderX) * 0.32;
    state.renderY += (state.targetY - state.renderY) * 0.32;

    const size = getCursorSize();
    cursor.style.transform = `translate3d(${state.renderX - size / 2}px, ${state.renderY - size / 2}px, 0)`;
    cursor.classList.toggle('is-discover', state.mode === 'discover');
    cursor.classList.toggle('is-hidden', state.mode === 'hidden');
    document.body.classList.toggle('custom-cursor-native', state.mode === 'hidden');

    state.frame = requestAnimationFrame(paintCursor);
  };

  const addTrail = (x, y) => {
    const now = performance.now();
    if (now - state.lastTrailAt < 42 || state.mode === 'hidden') return;
    state.lastTrailAt = now;

    const trail = document.createElement('span');
    trail.className = 'cursor-trail';
    trail.setAttribute('aria-hidden', 'true');
    trail.style.setProperty('--trail-x', `${x - 5.5}px`);
    trail.style.setProperty('--trail-y', `${y - 5.5}px`);
    trail.style.transform = `translate3d(${x - 5.5}px, ${y - 5.5}px, 0)`;
  ...
  const updateFromPointer = (event) => {
    ...
    if (!state.visible) {
      state.visible = true;
      state.renderX = target.x;
      state.renderY = target.y;
      document.body.classList.add('custom-cursor-ready');
    }
